...
6. §15's `Plugin.economy.costs: CostModel` names a type the Spec never defines and no milestone has implemented; M3-11 (the first real `Plugin` assembly) omitted it entirely rather than invent one, since inventing a cost model sideways through a plugin-assembly card would repeat the scope-creep pattern already rejected for ship shares/obligation (§5.3 of `docs/design/travel-and-import-v1.md`). Resolve either by defining `CostModel` for real once a milestone needs one, or by removing it from the interface. (Logged at the M3 retro.)
7. §15's `Plugin.persona`/`Plugin.dice` shapes (`PersonaDef`, `Lexicon`, `DiceConvention`, `CheckResult`, `DiceCheckInput`) existed only as untyped pseudocode until M3-11 gave them real types in `packages/engine/src/plugin-api/plugin.ts` — an extrapolation, not a Spec-derived shape. The critical-success/critical-failure convention (a natural 2 on the reported roll always fails, a natural 12 always succeeds, independent of the modified total) is likewise this session's own reading of "the standard 2d6 convention," not stated anywhere in the Spec. Revisit if a second plugin needs a different dice convention or persona shape. (Logged at the M3 retro.)
